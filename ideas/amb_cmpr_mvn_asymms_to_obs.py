...
def main():
    ...
    for asymm_idx in range(n_asymms):
        ...
        plt.savefig(
            f'test_normed_asymms_cmpr_{asymm_idx}.png', bbox_inches='tight', dpi=dpi)
...
def get_asymms(in_file):
    ...
    asymm = get_asymms_nd_v2_raw_cy(in_probs.copy(order='c'))

    # asymm_max = get_asymms_nd_v2_raw_cy(
    #     np.sort(in_probs, axis=0).copy(order='c'))
    #
    # for i in range(asymm_max.shape[1]):
    #     asymm_max[i,:] = asymm_max[i, i]
    #
    #     asymm[i,:] /= asymm_max[i, i]
