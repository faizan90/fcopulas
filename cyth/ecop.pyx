...
from .misc cimport (
    ...
    get_hist_nd,
    fill_hist_nd,
    fill_hist_nd_integral,
    fill_hist_nd_integral_v2,
    )
...
cpdef np.ndarray get_ecop_nd_empirical(
    ...
    n_vals = ranks.shape[0]
    n_dims = ranks.shape[1]

    cdf_vals = np.empty(n_vals, dtype=np.float64)
    ...
    for i in range(n_vals):
        ...
        cdf_vals[i] = ctr_tot / <DT_D> n_vals
...
cpdef np.ndarray get_ecop_nd_empirical_sorted(
    ...
    '''
    Ranks should be sorted in such a way that first a sort is done for
    the first column, then whenever there is a repeat in the first column,
    then according to the second column, then if there is repeat in the
    second column, then according to the third column and so on.

    The actual rank vector at each step is same as that from the original
    data but the sorting is what matters.

    Exploiting this allows us to exit the second loop earlier when all
    the ranks of the test step are greater than the reference step.
   
    Later on, the cdf values (non-exeedence probabilities) have to be
    resorted in to the original order.
    
    Even considering the sorting of ranks and then reordering them
    proved to be faster than taking the original order of the time
    series, at least in the case of discharge that I had.
    
    It seems to matter less as the dimensions increase.
    '''
    ...
    n_vals = ranks_sorted.shape[0]
    n_dims = ranks_sorted.shape[1]
    ...
    for i in range(n_vals):
        ...
        for j in range(n_vals):
            ctr_u = 0
            for k in range(n_dims):
                if ranks_sorted[j, k] <= ranks_sorted[i, k]:
                    ctr_u += 1
